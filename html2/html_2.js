...
for(var i=0;i<data.length;i++)
{
	if(data[i].item_code.substring(2,6)==="0000"){
		...
		newOption.innerHTML=data[i].item_name;
		L1.appendChild(newOption);
	}
}
function getL1()
{
	while(L2.children.length>1)L2.removeChild(L2.children[1]);
	while(L3.children.length>1)L3.removeChild(L3.children[1]);
	for(var i=0;i<data.length;i++)
	{
		if(data[i].item_name===L1.value)
		{
			for(var j=i+1;j<data.length;j++)
			{
				if(data[i].item_code.substring(0,2)===data[j].item_code.substring(0,2)&&(data[j].item_code.substring(4,6)==="00"))
				{
					var newOption=document.createElement('option');
					newOption.innerHTML=data[j].item_name;
					L2.appendChild(newOption);
				}
			}
		}
...
function getL2()
{
	...
	for(var i=0;i<data.length;i++)
	{
		if(data[i].item_name===L2.value&&data[i].item_code.substring(2,4)!=="00")
		{
			for(var j=i+1;j<data.length;j++)
			{
				if(data[i].item_code.substring(0,4)===data[j].item_code.substring(0,4))
				{
					var newOption=document.createElement('option');
					newOption.innerHTML=data[j].item_name;
					L3.appendChild(newOption);
				}
			}
		}
